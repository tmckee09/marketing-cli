# Brand Voice Mode Walkthroughs & Examples
...
10. **Write the file path correctly.** The profile saves to
    `./brand/voice-profile.md` — exactly that path. Both the `brand/`
    prefix and the `-profile` suffix matter for cross-skill references.
    Every downstream skill reads from this exact path; any deviation
