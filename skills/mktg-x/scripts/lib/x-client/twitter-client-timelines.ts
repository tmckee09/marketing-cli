...
export function withTimelines<TBase extends AbstractConstructor<TwitterClientBase>>(
  ...
  abstract class TwitterClientTimelines extends Base {
    ...
    private logBookmarksDebug(message: string, data?: Record<string, unknown>): void {
      const debugFlag = process.env.MKTG_X_DEBUG_BOOKMARKS ?? process.env.XTERMINAL_DEBUG_BOOKMARKS;
      if (debugFlag !== '1') {
        return;
      }
      if (data) {
        console.error(`[mktg-x][debug][bookmarks] ${message}`, JSON.stringify(data));
      } else {
        console.error(`[mktg-x][debug][bookmarks] ${message}`);
      }
    }
