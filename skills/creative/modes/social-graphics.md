...
**Model selection:** Handled by `brand/stack.md` — never hardcode model IDs in prompts or workflows.
...
**Do NOT hardcode model IDs.** Always refer to `brand/stack.md` for the current default image model and its verified API payload.

Check `brand/stack.md` for the currently configured default image model. The specific model varies per project.

### How to Call

1. Open `brand/stack.md`
...
5. Execute the API call via your configured generation provider

### Why the configured image model for Social Graphics

the configured image model is the single model for all social graphic generation — including text-heavy designs. It handles:
...
There is no need to route to a separate typography model. the configured image model handles text rendering natively. See the [Text Rendering Instructions](#text-rendering-instructions) section for how to get clean typography.

---

## Text Rendering Instructions

the configured image model has strong native typography capabilities. Use these instructions to get clean, legible text in social graphics.
...
For most social media use cases, the configured image model's native text rendering is more than sufficient. The fallback is only needed when every character must be letter-perfect.
...
the configured image model handles text rendering directly. Use the text rendering instructions from the [Text Rendering Instructions](#text-rendering-instructions) section.
...
Refer to `brand/stack.md` for the verified API payload. Insert your prompt and aspect ratio.

```
1. Open brand/stack.md
2. Copy the Image Generation verified payload
3. Insert prompt and aspect_ratio
4. Execute via generation API
...
8. **Text rendering works** — the configured image model handles headlines natively, no separate tool needed
