...
**Do NOT hardcode model IDs.** Always refer to `brand/stack.md` for the current default video model and its verified API payload.
...
```json
{
  "model": "{{comparison_video_model_from_stack}}",
  "input": {
    "prompt": "{{presenter_prompt}}",
    "seconds": 8,
...
```json
{
  "model": "{{lip_sync_model_from_stack}}",
  "input": {
...
```
Model: {{default_video_model from brand/stack.md}}
...
```
Model: {{comparison_video_model from brand/stack.md}}
...
```
Model: {{lip_sync_model from brand/stack.md}}
...
*This mode is part of the Vibe Marketing Skills v2 creative engine. Model configurations are maintained in `brand/stack.md`. When model schemas change upstream, MODEL_REGISTRY.md is updated first, then this mode file is updated to match.*
